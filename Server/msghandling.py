...
def handle_CreateRoomReq(resources, sender_conn, msg):
    try:
        ...
        room = gr.Room(resources['config'],
                       msg['user_name'], sender_conn, room_code)
...
def handle_JoinRoomReq(resources, sender_conn, msg):
    try:
        ...
        info = 'Username {} already taken in room with code {}'.format(
            msg['user_name'], msg['room_code'])
        send_NOT_OK_JoinRoomResp_with_info(
            sender_conn, resources['config'], info)
...
def handle_ExitClientReq(resources, sender_conn, msg):
    try:
        ...
        if removed:
            ...
            logging.info('[handle_ExitClientReq] Removed {} from room with code {}'.format(
                user_name, code))
        else:
            logging.info('[handle_ExitClientReq] User {} not found in room with code {}'.format(
                user_name, code))

        if room.num_of_members() == 0:
            del rooms[code]
            logging.info(
                '[handle_ExitClientReq] Room with code {} deleted (0 players)'.format(code))
    except:
        logging.error(
            '[handle_ExitClientReq] Error ocured when handling message {}'.format(msg))
...
def handle_StartGameReq(resources, sender_conn, msg):
    try:
        ...
        traceback.print_exc()
        logging.error(
            '[handle_StartGameReq] Error ocured when handling message {}'.format(msg))
        resp = mc.build_start_game_resp_not_ok('Something went wrong -_-')
        nw.send(sender_conn, resp, resources['config'])
