...
class CreateRoomReqHandler:
    ...
    def handle(self, sender_conn, msg):
        try:
            ...
            room = Room(self.server_config, msg.room_name, msg.user_name, sender_conn)
            ...
            logging.debug('[CREATE_ROOM_REQ_HANDLER] Created new room with room code {}'.format(room_code))

        except:
            traceback.print_exc()
            logging.error('[CREATE_ROOM_REQ_HANDLER] Error ocured when handling message')
            resp = messages.CreateRoomResp()
            resp.status = 'NOT_OK'
            msg_handling.send(sender_conn, resp, self.server_config)
