...
class ConnectionHandler(QtCore.QObject):
    ...
    def handle_UpdateScoreboardBc(self, received_msg):
        logging.debug('[MESSAGE DISPATCHER] handling handle_UpdateScoreboardBc {}'.format(received_msg))
    ...
    def send_word_selection_resp(self, user_name, room_code, selected_word):
        logging.debug('[SENDING MESSAGE] WordSelectionResp, selected word = {}'.format(selected_word))
        word_selection_resp = {
    ...
    def send_undo_last_stroke_req(self, user_name, room_code):
        undo_last_stroke_req = {'msg_name': 'UndoLastStrokeReq', 'user_name': user_name, 'room_code': room_code}
    ...
    def send_clear_canvas_req(self, user_name, room_code):
        clear_canvas_req = {'msg_name': 'ClearCanvasReq', 'user_name': user_name, 'room_code': room_code}
        SocketMsgHandler.send(self.conn, clear_canvas_req, self.server_config)

    def send_finish_game_req(self, user_name, room_code):
        finish_game_req = {'msg_name': 'FinishGameReq', 'user_name': user_name, 'room_code': room_code}
