...
class ConnectionHandler(QtCore.QObject):
    ...
    def __init__(self):
        ...
        self.receiver_thread = threading.Thread(target=self.receive, args=(self.conn, self.server_config))
    ...
    def _load_config_file(self):
        try:
            ...
            logging.error('[LOADING CONFIG FILE] Error occured when loading configuration file!')
            exit()
    ...
    def handle_ChatMessageBc(self, received_msg):
        logging.debug('[MESSAGE DISPATCHER] handling ChatMessageBc: {}'.format(received_msg))
    ...
    def handle_StartGameResp(self, received_msg):
        logging.debug('[MESSAGE DISPATCHER] handling StartGameResp, STATUS {}'.format(received_msg['status']))
        if received_msg['status'] == 'NOT_OK':
            PopUpWindow(received_msg['info'], 'ERROR')

    def handle_StartGameBc(self, received_msg):
        logging.debug('[MESSAGE DISPATCHER] handling StartGameBc, Artist: {}'.format(received_msg['artist']))
    ...
    def handle_ArtistPickBc(self, received_msg):
        logging.debug('[MESSAGE DISPATCHER] handling ArtistPickBc, Artist: {}'.format(received_msg['artist']))
        self.artist_change_signal.emit(received_msg)

    def handle_WordSelectionReq(self, received_msg):
        logging.debug('[MESSAGE DISPATCHER] handling WordSelectionReq, Word List: {}'.format(received_msg['word_list']))
    ...
    def handle_GuessWordMessageResp(self, received_msg):
        logging.debug('[MESSAGE DISPATCHER] handling GuessWordMessageResp')
        if received_msg['verdict']:
            ...
            logging.debug(
                '[GUESS CORRECT] {} has guessed the word and gained {} points'.format(
                    received_msg['user_name'], received_msg['score_awarded']
                )
            )
    ...
    def handle_UnrecognizedMessage(self, received_msg):
        logging.debug('[MESSAGE DISPATCHER] No defined handler for message: {}'.format(received_msg))
    ...
    def send_join_room_req(self, user_name, room_code):
        send_join_room_req_msg = {'msg_name': 'JoinRoomReq', 'user_name': user_name, 'room_code': room_code}
    ...
    def send_exit_client_req(self, user_name, room_code):
        notify_server_about_leaving = {'msg_name': 'ExitClientReq', 'user_name': user_name, 'room_code': room_code}
    ...
    def send_start_game_req(self, user_name, room_code):
        start_game_req = {'msg_name': 'StartGameReq', 'user_name': user_name, 'room_code': room_code}
    ...
    def send_draw_stroke_req(self, stroke_coordinates):
        draw_stroke_req = {'msg_name': 'DrawStrokeReq', 'stroke_coordinates': stroke_coordinates}
        SocketMsgHandler.send(self.conn, draw_stroke_req, self.server_config)

    def send_undo_last_stroke_req(self):
        undo_last_stroke_req = {'msg_name': 'UndoLastStrokeReq'}
        SocketMsgHandler.send(self.conn, undo_last_stroke_req, self.server_config)

    def send_clear_canvas_req(self):
        clear_canvas_req = {'msg_name': 'ClearCanvasReq'}
        SocketMsgHandler.send(self.conn, clear_canvas_req, self.server_config)

    def send_guess_word_message_req(self, word_guess):
        guess_word_message_req = {'msg_name': 'GuessWordMessageReq', 'word_guess': word_guess}
        SocketMsgHandler.send(self.conn, guess_word_message_req, self.server_config)

    def send_finish_game_req(self, user_name):
        finish_game_req = {'msg_name': 'FinishGameReq', 'user_name': user_name}
